...
from PySide6.QtWidgets import QButtonGroup, QFrame, QHeaderView, QHBoxLayout, QLabel, QPushButton, QScrollArea, QSlider, QStackedLayout, QTableView, QVBoxLayout, QWidget
...
class TagOverview(QFrame):
    filter_changed = Signal(list, str)
    ...
    def __init__(self, parent=None) -> None:
        ...
        top.addWidget(self.cloud_button)
        top.addWidget(self.table_button)
        ...
        action_row.addWidget(self.sort_count_button)
        action_row.addWidget(self.sort_alpha_button)
        action_row.addSpacing(10)
        self.relation_label = QLabel("筛选关系")
        action_row.addWidget(self.relation_label)
        action_row.addWidget(self.or_button)
        action_row.addWidget(self.and_button)
        action_row.addSpacing(10)
    ...
    def _invert_visible(self) -> None:
        visible = {row.tag for row in self.visible_rows}
    ...
    def _emit_filter(self) -> None:
        self._update_label()
        self.filter_changed.emit(sorted(self.selected), self._mode)
    ...
    def _update_label(self) -> None:
        ...
        text = f"Current filter: {relation} · {shown}{suffix}" if self.language == "en" else f"当前筛选：{relation} · {shown}{suffix}"
        self.selected_label.setText(text)
        prefix = f"Current filter: {relation} · " if self.language == "en" else f"当前筛选：{relation} · "
        self.selected_label.setToolTip(prefix + (", ".join(tags) if self.language == "en" else "，".join(tags)))
