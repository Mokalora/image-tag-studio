    ...
    def filtered_files(self) -> list[TagFile]:
        ...
        if self.filter_tags:
            ...
            if self.filter_mode == "and":
                result = [item for item in result if selected.issubset(set(item.tags) | set(item.virtual_tag_keys))]
            else:
                result = [item for item in result if selected.intersection(set(item.tags) | set(item.virtual_tag_keys))]
        return result
